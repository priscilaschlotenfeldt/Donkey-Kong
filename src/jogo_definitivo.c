...
typedef struct textura
{
    ...
    Texture2D texParede;
    Texture2D texEscada;
    Texture2D texObjetivo;
    Texture2D desenhaSafeZone;
    Texture2D texSpawnpoint;
} TEXTURA;
...
{
    // Se pisar, personagem mata o inimigo
    if (personagem.linha >= (m->linha - (TAMANHO * 1.2)) && personagem.linha <= (m->linha - (TAMANHO * 0.5)) && personagem.coluna <= (m->coluna + TAMANHO * 0.7) && personagem.coluna >= (m->coluna - TAMANHO * 0.7))
    {
        ...
        *m = inimigoMorto; // inimigo morto == constante, quando é pisado, ele é teleportado para fora de onde o mapa mostra
        pontos += 100;    // acrescenta pontos
    }

    // Se encostar, personagem perde uma vida (até perder todas)
    else if ((invulnerabilidade < 0) && personagem.linha >= (m->linha - (TAMANHO * 0.8)) && personagem.linha <= (m->linha + (TAMANHO * 0.5)) && personagem.coluna <= (m->coluna + (TAMANHO * 0.7)) && personagem.coluna >= (m->coluna - (TAMANHO * 0.7)))
    {
        ...
        invulnerabilidade = (60 * 3); // frames por segundo * segundos invuneravel
    }

    // mov dir ou troca
    // pergunta se o movimento é para x lado, ve se tem chao ou texParede, se tiver ele muda de direção, se não segue em frente tal qual um robo aspirador
...
void desenhaObjeto(OBJETO *m, TEXTURA *tex)
{
    ...
    {
        desenhaTex(tex->texPlataforma, (Rectangle){m->coluna * TAMANHO, m->linha * TAMANHO, TAMANHO, TAMANHO});
        desenhaChao(m);
    }
    // se for texParede
    else if (m->tipo == 'W')
    {
        desenhaTex(tex->texParede, (Rectangle){m->coluna * TAMANHO, m->linha * TAMANHO, TAMANHO, TAMANHO});
        desenhaParede(m);
    }
    // se for escada
    else if (m->tipo == 'D' || m->tipo == 'H' || m->tipo == 'S')
    {
        ...
        desenhaTex(tex->texEscada, (Rectangle){m->coluna * TAMANHO, m->linha * TAMANHO, TAMANHO, TAMANHO});
        desenhaEscada(m);
    }
    // se for o objetivo
    else if (m->tipo == 'F')
    {
        ...
        desenhaTex(tex->texObjetivo, (Rectangle){m->coluna * TAMANHO, m->linha * TAMANHO, TAMANHO, TAMANHO});
    ...
    {
        desenhaTex(tex->desenhaSafeZone, (Rectangle){m->coluna * TAMANHO, m->linha * TAMANHO, TAMANHO, TAMANHO});
        desenhaSafeZone(m);
    }
}
...
bool executarJogo(void)
{
    ...
    texBarril = CarregaTex("assets/images/sprites/spriteBarrilFrente.png");
    ...
    for (int i = 0; ((i < QUANTIDADE_MAPAS) && !morte); i++)
    {
        ...
        do
        {
            ...
            desenhaTex(tex.texSpawnpoint, GetRect(spawnpoint.coluna, spawnpoint.linha));
    ...
    UnloadTexture(tex.texPlataforma);
    UnloadTexture(tex.texParede);
    UnloadTexture(tex.texEscada);
    UnloadTexture(tex.texObjetivo);
    UnloadTexture(tex.desenhaSafeZone);
    UnloadTexture(tex.texSpawnpoint);
